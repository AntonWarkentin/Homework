...
namespace Home_6
{
    internal class Ophthalmologist : Physician
    {
        ...
        {
            Name = name;
            doctorsSpecialisation = IllnessType.Eyes;
        }

        public override void Treat(Patient patient)
        {
            Console.WriteLine($"Treating patient {patient.Name} by Ophthalmologist {this.Name} starts:");
            base.Treat(patient);            
            Console.WriteLine($"Ophthalmologist {this.Name} has finished treating patient {patient.Name}.");
            Console.WriteLine();
        }
    }
}
